...
const ContractorDashboard = ({ navigation, route }) => {
  ...
  useEffect(() => {
    // If navigated here with showAllJobs flag, ensure the jobs container is visible
    if (route?.params?.showAllJobs) {
      setShowJobsContainer(true);
      // Clear the param so repeated navigations don't trigger it again
      try { navigation.setParams({ showAllJobs: false }); } catch (e) { /* ignore if not available */ }
    }
  }, [route]);

  // Setup Firestore listener for all jobs
  useEffect(() => {
  ...
  // Remove jobs that have already been completed from the visible jobs list
  const visibleJobs = jobs.filter(j => (j.status || '').toString().toLowerCase() !== 'completed');
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              setShowNavigationModal(false);
              // Navigate to NavigationScreen and ask it to show dumpsters
              ...
              Alert.alert(
                '✅ Pickup Confirmed!',
                'Job status updated to in progress. Nearby dumpsters list is available on the navigation screen.',
                [{ text: 'OK' }]
              );
            } catch (error) {
  ...
  // On mount, restore contractor's online state from Firestore so navigating to this
  // screen does not accidentally make the driver appear offline. If the contractor
  // is marked online in Firestore, resume location tracking and session timer.
  useEffect(() => {
    const restoreOnlineState = async () => {
      ...
      try {
        const contractorRef = doc(db, 'contractors', auth.currentUser.uid);
        const contractorSnap = await getDoc(contractorRef);
        if (contractorSnap && contractorSnap.exists()) {
          const data = contractorSnap.data();
          if (data.isOnline) {
            // Keep the driver online when arriving at this screen
            setIsOnline(true);

            // Start location tracking and session timer if not already running
            try {
              await startLocationTracking();
            } catch (err) {
              console.warn('Failed to start location tracking on restore:', err);
            }

            // Start the session timer UI - we don't attempt to reconstruct the exact
            // previous start time here (that is stored in Firestore as lastLoginTime),
            // but starting the timer will reflect continued online state in the UI.
            try {
              startSessionTimer();
            } catch (err) {
              console.warn('Failed to start session timer on restore:', err);
            }
          }
        }
      } catch (error) {
        console.error('Error restoring contractor online state:', error);
      }
    };
